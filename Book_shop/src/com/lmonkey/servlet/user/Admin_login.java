...
@WebServlet(name = "adminLogin",urlPatterns = "/manage/adminLogin")
		...
		if (count > 0){
			...
			try {
				...
				if (user.getUSER_STATUS() == 2){
					...
					response.sendRedirect("/Book_shop/web/manage/Admin_index.jsp");
				}else {
					response.sendRedirect("/Book_shop/web/index.jsp");
				}
			} catch (SQLException e) {
				e.printStackTrace();
			}
